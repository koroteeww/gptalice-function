...
def write_process_status(key, data):
    ...
    try:
        s3.put_object(Bucket=config['s3_bucket'], Key=key, Body=json_data, ContentType='text/html;charset=utf-8')
...
def check_processing_objects():
    ...
    for obj in objects:
        ...
        if 'response' in result_data and 'chunks' in result_data['response']:
            ...
            try:
                s3.put_object(Bucket=config['s3_bucket'], Key=result_text_key, Body=result_text_encoded, ContentType='text/html;charset=utf-8')
        ...
        try:
            s3.put_object(Bucket=config['s3_bucket'], Key=key, Body=json.dumps(body_complete), ContentType='text/html;charset=utf-8')
