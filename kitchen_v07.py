...
def step3_stt_whisper(mp3path)->str:
    #using whisper translate
    #print("going to speech to text "+str(mp3path))
    text=""
    # Read the audio file as binary data
    with open(mp3path, 'rb') as f:  
        res = client.audio.translations.create(file=f,model="whisper-1",response_format="text", prompt=f" translate from {langfrom} and please try your best to make the result with as closest audio duration to original as possible")      
        #res = client.audio.transcriptions.create(model='whisper-1',file=f, language=whisperlang)
        # Print the transcribed text
        print('Whisper translations text='+res)
        text=res
    #save whisper log
    with open("whisperLogT.txt","a", encoding="utf-8") as log:
        log.write(f"Whisper response to {mp3path} is: {text} \r\n")
...
def main():
    ...
    if ( len(new_files) >0):
        for mp4file in new_files:
            ...
            # attempt to get full text from whisper
            #            
            
            #20/12/2023 new idea, do full voiceover and THEN apply on video synchronizing lips with words
            #break
            ...
            for chunk in mp3list:
                ...
                if (checkstt_ok(translation_stt, langfrom) and is_null_or_whitespace(translation_stt)==False):
                    ...
                    #text2 = step3_stt_whisper(chunk)
                    #texttranslated.append(text2)
                else:
            ...
            for llang in lang_to:
                ...
                alltext=""
                for text in textstt: 
                    translated_text = step4_translate(text,lang=llang,source=langfrom)
                    if (isBlank(translated_text)): continue              
                    genaratedaudio = step5_generatefromtranslate(translated_text,lang=llang,vvoice=voice)
                    #normalize wav
                    normalizedaudio = normalizewav(genaratedaudio)
                    gen_audios.append(normalizedaudio)
                    alltext = alltext+translated_text+". "
                #join audios
                print("-Concat generated audios "+str(len(gen_audios)))
                totalaudio=ffconcat(gen_audios,pauses,llang)
                newvideofile = step6_audiotovideo(totalaudio,mp4file,lang=llang)
                print(f"RESULT of lang {llang} ="+newvideofile)
                file_audio = step5_generatefromtranslate(alltext,llang,voice)
                print("whole translated audio: "+file_audio)
